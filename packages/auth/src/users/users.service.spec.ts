...
describe("UsersService", () => {
  ...
  let userRoleRepo: Repository<UserRoleRelationship>;
  let attemptRepo: Repository<Attempt>;
  ...
  beforeEach(async () => {
    ...
    userRoleRepo = {
      ...
      createQueryBuilder: jest.fn(() => ({
        where: jest.fn(() => ({ delete: () => ({ execute: jest.fn() }) })),
      })),
    } as any;
    attemptRepo = {
      createQueryBuilder: jest.fn(() => ({
        where: jest.fn(() => ({
          ansWhere: () => ({ execute: jest.fn(() => []) }),
        })),
      })),
      insert: jest.fn(),
      delete: jest.fn(),
  ...
  it("should add user", async () => {
    ...
    service = new UsersService(userRepo, passwordRepo, userRoleRepo, attemptRepo);
  ...
  it.skip("should throw when it fails to add", async () => {
    expect.assertions(3);
    userRepo = {
      ...userRepo,
      findOneBy: jest.fn(() => null),
